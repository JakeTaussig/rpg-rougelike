...
enum State {WAITING_FOR_INPUT, PLAYER_ATTACK_INFO, ENEMY_ATTACK, ENEMY_ATTACK_INFO}
...
func _ready() -> void:
	...
	if battle_participants[turn_order_index].is_player:
		_update_state(State.WAITING_FOR_INPUT)
	else:
...
func _update_state(new_state: State):
	...
	if old_state == State.PLAYER_ATTACK_INFO || old_state == State.ENEMY_ATTACK_INFO:
		_render_hp()
